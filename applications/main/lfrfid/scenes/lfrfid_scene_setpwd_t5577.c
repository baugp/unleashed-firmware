...
static void lfrfid_set_t5577_password_and_config_to_EM(LfRfid* app) {
    ...
    const uint32_t em_config_block_data =
        ...
        0b00000000000101001000000001010000; //pwd config block
    ...
    LFRFIDT5577 data;
    data.block[7] = default_passwords[i];
    data.mask = 1 << 7; //write password (block 7)
    t5577_write_with_mask(&data, 0, false, 0);

    // Enable Password
    data.block[0] = em_config_block_data;
    data.mask = 1 << 0; //write config (block 0)
    t5577_write_with_mask(&data, 0, false, 0);
...
void lfrfid_scene_setpwd_t5577_on_enter(void* context) {
    LfRfid* app = context;
    Popup* popup = app->popup;

    lfrfid_set_t5577_password_and_config_to_EM(app);
...
bool lfrfid_scene_setpwd_t5577_on_event(void* context, SceneManagerEvent event) {
    LfRfid* app = context;
    bool consumed = false;
...
void lfrfid_scene_setpwd_t5577_on_exit(void* context) {
    LfRfid* app = context;
    popup_reset(app->popup);
